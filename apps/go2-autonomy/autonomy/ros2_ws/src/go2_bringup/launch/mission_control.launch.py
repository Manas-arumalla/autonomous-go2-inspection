...
def generate_launch_description():
    return LaunchDescription([
        # WendyOS/real-robot: use_sim_time defaults FALSE (no /clock on the Go2); workspace defaults to the
        # in-container colcon workspace so save_map can find the map helper scripts.
        DeclareLaunchArgument("use_sim_time", default_value="false"),
        DeclareLaunchArgument("zones_file", default_value="/maps/facility_inspection_zones.yaml"),
        DeclareLaunchArgument("map_name", default_value="facility_inspection"),
        DeclareLaunchArgument("workspace", default_value="/autonomy_ws"),
        Node(

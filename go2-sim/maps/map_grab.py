#!/usr/bin/env python3
"""Grab the live RTAB-Map 2D occupancy grid (/map) and save it to a .npz (grid,res,origin) + print
per-room coverage. Run this AFTER the robot has mapped the facility, while the SLAM stack is still up.
  python3 map_grab.py out.npz
Then segment zones:  python3 ../go2_ws/src/go2_zones/go2_zones/zone_segmenter.py out.npz
And make the mission map_server map:  python3 npz_to_map.py out.npz   (-> out.pgm + out.yaml)
"""

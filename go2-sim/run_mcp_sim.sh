#!/usr/bin/env bash
# Launch the Go2 SIM mission-control MCP server for Claude CLI (stdio).
#
# Claude CLI spawns this script. It sources ROS 2 + the sim workspace (so rclpy + the ZoneTask interface
# are importable) and matches the sim's DDS env (so it DISCOVERS the running mission_control), then runs
# the server in the venv that has fastmcp. Register it once with:
#
#     claude mcp add go2-sim -- "/home/manas-reddy/Downloads/EE26 Hackathon/go2-sim/run_mcp_sim.sh"
#
...
WS="/home/manas-reddy/Downloads/EE26 Hackathon/go2-sim/go2_ws"
...
# mission_control (verified). We only pin the domain (Claude may spawn us with a minimal env). We do NOT
...
# uncomment the two lines below to match your sim terminals:

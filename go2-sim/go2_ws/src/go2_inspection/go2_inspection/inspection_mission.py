...
VENV_PY = os.path.expanduser("~/gauge_venv/bin/python")
...
class Mission(Node):
    def __init__(self):
        ...
        self.zones_file = os.path.expanduser(self.declare_parameter(
            "zones_file", "~/.go2_maps/facility_inspection_zones.yaml").value)
...
def inspect_zone(zone_id, zones_file, do_read=True):
    ...
    zone_sweeper + FastSAM. Both produce ~/gauges/<zone>/gauges.json in the same schema."""
    ...
    if os.environ.get("INSPECT_LEGACY") == "1":
        sweeper = ["ros2", "run", "go2_inspection", "zone_sweeper", "--ros-args",
                   "-p", "use_sim_time:=true", "-p", "skip_nav:=true", "-p", "find_wall:=true",
        ...
        # zone_wall_follower now runs YOLOE LIVE during the scan and writes the crops + detections.json +
        # gauges.json itself, so there is NO separate post-pass segmenter. (Running yoloe_segmenter here
        # would find no panoramas -- we dropped panorama stitching -- and overwrite gauges.json with an
        # empty result, clobbering the live captures.)
...
def main(args=None):
    ...
    for z in cands:
        ...
        gj = inspect_zone(zid, m.zones_file, m.read)

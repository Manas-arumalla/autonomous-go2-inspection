...
def generate_launch_description():
    ...
    # champ_config removed (EE26 Go2): this build supplies joints/links/gait via go2_config and
    # passes them as launch args (joints_map_path/links_map_path/gait_config_path), so the eager
    # FindPackageShare("champ_config") here used to crash the whole launch. Only champ_description
    # is needed (rviz + default-model fallbacks).
    ...
    base_to_footprint_ekf = Node(
        ...
        parameters=[
            ...
            {"use_sim_time": LaunchConfiguration("use_sim_time")},
            {"publish_tf": True},   # EE26: EKFs publish odom->base_footprint->base_link for SLAM/Nav2
            os.path.join(
